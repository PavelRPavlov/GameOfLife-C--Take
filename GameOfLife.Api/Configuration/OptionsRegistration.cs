...
/// (unparseable rule, unusable coordinate type, empty or Production-unsafe localhost origins) fails the
...
internal sealed class GameOptionsValidator : IValidateOptions<GameOptions>
{
    public ValidateOptionsResult Validate(string? name, GameOptions options)
    {
        ...
        if (!Universe.TryParseCoordinateType(options.UniverseAxisSize, out _))
            failures.Add(
                $"{GameOptions.SectionName}:{nameof(GameOptions.UniverseAxisSize)} '{options.UniverseAxisSize}' is not a " +
